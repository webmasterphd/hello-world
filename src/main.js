...
const assignments = [
  { icon: 'target', title: 'Pacing refinement', subtitle: '15-minute session focused on property ownership questions', cta: 'Begin assignment' },
  { icon: 'audio', title: 'Listen while walking', subtitle: 'OLD CAR podcast segment prescribed for today', cta: 'Play audio' },
  { icon: 'concierge', title: 'Confer with Sterling', subtitle: 'Ask for a walkthrough of your two missed fiduciary duty items', cta: 'Confer in writing' },
];

const iconMarkup = {
  target: '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"></circle><circle cx="12" cy="12" r="3"></circle></svg>',
  audio: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 13a8 8 0 0 1 16 0"></path><path d="M6 13v5"></path><path d="M18 13v5"></path><path d="M6 18h3v-6H6z"></path><path d="M15 18h3v-6h-3z"></path></svg>',
  concierge: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m12 3 2.4 6.6L21 12l-6.6 2.4L12 21l-2.4-6.6L3 12l6.6-2.4z"></path></svg>'
};
    ...
    <main class="dashboard-shell">
      ...
      <section class="grid-layout">
        <div class="left-stack">
          <section class="glass intercept-card"><div class="section-heading"><div><p class="eyebrow">Today’s intercepts</p><h3>Review at the precise moment memory can be strengthened.</h3></div><span class="section-icon">✦</span></div><div class="intercept-list">${intercepts.map(item => `<article class="intercept-row"><strong>${item.type}</strong><div><h4>${item.label}</h4><p>${item.note}</p></div><span>${item.time}</span></article>`).join('')}</div></section>
        ...
        <aside class="right-stack">
          ...
          <section class="glass assignment-card"><p class="eyebrow">Concierge Desk assignment</p>${assignments.map(item => `<article><span class="assignment-icon">${iconMarkup[item.icon]}</span><div><h4>${item.title}</h4><p>${item.subtitle}</p><button>${item.cta}</button></div></article>`).join('')}</section>
        </aside>
      </section>
    </main>`;
